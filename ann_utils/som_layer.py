...
class SOMLayer(object):
    ...
    def get_reshaped_map(self, sz):
        m = tf.reshape( ( self.map - tf.reduce_min( self.map ) ) / ( tf.reduce_max( self.map ) - tf.reduce_min( self.map ) ), [self.m, self.m, sz] )
        return tf.reduce_mean( m, axis = 2, keep_dims = True ) 
    ...
    def __call__(self, x, is_training=False):
        ...
        self.map = tf.compat.v1.get_variable( "map_{}".format( self.name ), 
                                    initializer = tf.random_normal( shape = [ self.m * self.n, dim ], stddev = 0.05 ), 
                                    dtype = tf.float32, trainable = False )

        self.iter = tf.compat.v1.get_variable( "iter_{}".format( self.name ), 
